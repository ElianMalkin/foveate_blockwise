...
def make_kernel(S, num_regions):
	...
	__device__ __constant__ int c_c[400];

	__global__ void gridmake_bw(uint8_t *d_out, const unsigned int *blur_grid, const unsigned int blockW, const unsigned int blockH,
								const unsigned int W, const unsigned int H, const int width_shift, const int height_shift) 
	{
		unsigned int grid_pos = blockIdx.y * gridDim.x + blockIdx.x;
		unsigned int c_c_ind = blur_grid[grid_pos];
		unsigned int color = c_c[c_c_ind];

		const int blockStartCol = blockIdx.x * blockW - width_shift;
		const int blockStartRow = blockIdx.y * blockH - height_shift;

		int iRow = blockStartRow + threadIdx.y;
		int iCol = blockStartCol + threadIdx.x; 

		if( iRow < H && iCol < W && iRow >= 0 && iCol >= 0 ) {
			unsigned int iPixelPos = iRow * W + iCol; 
			d_out[iPixelPos] = color;
		}
	}
...
def mouse_draw(event, x, y, flags, img):
	global drawing, brush_thickness, brush_color
	if event == cv.EVENT_LBUTTONDOWN:
		drawing = True
	elif event == cv.EVENT_MOUSEMOVE and drawing:
		cv.circle(img, (x, y), brush_thickness, (brush_color, brush_color, brush_color), -1)
...
def draw_retina():
	...
	cv.namedWindow('Drawing window', cv.WINDOW_NORMAL)
	blank_retina = np.zeros((H, W, 3), dtype=np.uint8)
	cv.setMouseCallback('Drawing window', mouse_draw, blank_retina)

	while(1):
		cv.imshow("Drawing window", blank_retina)
		k = cv.waitKey(20)
		if k == ord('s'):
			break
		elif k == 46 and brush_thickness <= 90:
			brush_thickness += 10
		elif k == 44 and brush_thickness >= 20:
			brush_thickness -= 10
		elif k == 49:
			brush_color = 255
		elif k == 50:
...
def sample_blurmap(host_output):
	map_max = np.amax(host_output)
	map_min = np.amin(host_output)
	...
	for i in range(gridH):
		...
		for j in range(gridW):
			...
			val = host_output[row, col, 0]
			grid_val = np.int32(np.rint((val - map_min)/(map_max - map_min) * 400))

			blur_grid[i, j] = grid_val
			b.add(grid_val)

	ordered_b = list(b)
	ordered_b.sort(reverse=True)

	num_regions = len(ordered_b)

	for i in range(gridH):
		for j in range(gridW):
			blur_grid[i,j] = ordered_b.index(blur_grid[i,j])

	blur_grid_gpu = cuda.mem_alloc(blur_grid.nbytes)
	cuda.memcpy_htod(blur_grid_gpu, blur_grid)

	return blur_grid_gpu, ordered_b, num_regions, r_w_c, r_h_c

def illu_blurgrid(ordered_b, mod, host_output, blur_grid_gpu):
	map_max = np.amax(host_output)
	map_min = np.amin(host_output)
	...
	b_color = np.array(ordered_b)
	b_color = np.int32(b_color/400 * (map_max - map_min) + map_min)

	c_c_pt = mod.get_global("c_c")[0]
	cuda.memcpy_htod(c_c_pt, b_color)

	illu_img = np.ones((H,W), dtype="uint8")
	illu_gpu = cuda.mem_alloc(illu_img.nbytes)
	illu_output = np.empty_like(illu_img)

	illu_func = mod.get_function("gridmake_bw")
	illu_block=(fragmentW, fragmentH, 1)
	illu_func.prepare(("P", "P", "i", "i", "i", "i", "i", "i"))

	grid = (gridW, gridH)
	illu_func.prepared_call(grid, illu_block, illu_gpu, blur_grid_gpu, np.int32(fragmentW), np.int32(fragmentH), np.int32(W), np.int32(H), width_shift_c, height_shift_c)
	cuda.memcpy_dtoh(illu_output, illu_gpu)

	cv.imshow("Drawing window", illu_output)
	cv.waitKey(0)

def make_sigmas_map(ordered_b):
	ordered_b = np.array(ordered_b, dtype="int32")

	fc_deg = 1/((400 - ordered_b)/400 * 100 + e2) * (e2/alpha * np.log(1/CT_0))

	for i in range(fc_deg.shape[0]):
		if fc_deg[i] > max_cyc_per_deg:
			fc_deg[i] = max_cyc_per_deg

	sig_f = 0.5 * fc_deg/max_cyc_per_deg
	sigma_l = 1/(2*np.pi*sig_f)
...
def main():
	...
	gridW = math.ceil(W/fragmentW)
	gridH = math.ceil(H/fragmentH)

	blur_grid = np.ones((gridH, gridW), dtype="int32")
	blur_grid_gpu = cuda.mem_alloc(blur_grid.nbytes)
	cuda.memcpy_htod(blur_grid_gpu, blur_grid)
	...
	img_padded = replication_pad(retina, W, H, S, paddedW, paddedH)
	transfer_input_img(img_padded, img_pinned, img_gpu)

	run_foveation(mod, gridW, gridH, img_gpu, blur_grid_gpu, np.int32(paddedW), np.int32(paddedH), S, result_gpu, W, 0, 0, 0, 0)
	transfer_result(host_output, result_gpu)

	# cv.namedWindow('blurmap window', cv.WINDOW_NORMAL)
	# cv.imshow("blurmap window", host_output)
	# k = cv.waitKey(0)
	# ____
	...
	blur_grid_gpu, ordered_b, num_regions, r_w_c, r_h_c = sample_blurmap(host_output)
	...
	sigma_l = make_sigmas_map(ordered_b)
	l_ker_size = calc_ker_size(sigma_l[-1])
	S = int((l_ker_size-1)/2)
	...
	#illu_blurgrid(ordered_b, mod, blurmap, blur_grid_gpu)
